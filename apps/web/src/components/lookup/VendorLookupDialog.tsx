...
import { VENDORS, type VendorRow } from "@/components/lookup/sample-data";
...
export function VendorLookupDialog({ open, onOpenChange, onSelect }: Props) {
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(720px,calc(100vw-2rem))] max-h-[80vh] -translate-x-1/2 -translate-y-1/2 rounded border border-neutral-40 bg-white shadow-flyout focus:outline-none flex flex-col">
          ...
          <Dialog.Description className="sr-only">{t("lookup.vendor.description")}</Dialog.Description>
          ...
          <div className="px-4 py-3 border-b border-neutral-30">
            <div className="relative">
              ...
              <input
                ...
                placeholder={t("lookup.vendor.placeholder")}
                value={query}
          ...
          <div className="flex-1 overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="bg-neutral-10 text-neutral-130 text-xs font-semibold sticky top-0">
                <tr>
                  <th className="text-left px-4 py-2">{t("lookup.vendor.cols.no")}</th>
                  <th className="text-left px-4 py-2">{t("lookup.vendor.cols.name")}</th>
                  <th className="text-left px-4 py-2">{t("lookup.vendor.cols.contact")}</th>
                  <th className="text-left px-4 py-2">{t("lookup.vendor.cols.country")}</th>
                  <th className="text-left px-4 py-2">{t("lookup.vendor.cols.lastTxn")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-neutral-20">
                {filtered.map((v) => (
                  <tr
                    key={v.no}
                    className={cn(
                      "row-standard hover:bg-brand-50/40 cursor-pointer",
                    )}
                    onClick={() => {
                      onSelect(v);
                      onOpenChange(false);
                  ...
                  >
                    <td className="px-4 font-mono text-xs">{v.no}</td>
                    <td className="px-4">{v.name}</td>
                    <td className="px-4 text-neutral-130">{v.contact}</td>
                    <td className="px-4 text-neutral-130">{v.country}</td>
                    <td className="px-4 text-neutral-130 font-mono text-xs">{v.lastTxn}</td>
                  </tr>
                ))}
                {filtered.length === 0 && (
                  <tr>
          ...
          <div className="px-4 py-2.5 text-xs text-neutral-130 border-t border-neutral-30">
            {t("lookup.vendor.footer", { count: filtered.length })}
