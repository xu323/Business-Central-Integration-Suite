...
本作品集聚焦於 **Purchases & Payables（採購與應付）** 中的「請購單 → 採購核准 → Purchase Order 同步」流程。
...
本專案的 `Approval Mgt.` codeunit 用簡化版自製狀態機，方便面試解釋；同時在文件中說明真實 BC Workflow 如何接入。
...
- README 註明限制，並指引面試官如何用本機 BC sandbox 真正編譯
...
| 核准 | Workflow + Approval Entry | 簡化狀態機 + Audit Log | 方便面試展示完整生命週期 |
